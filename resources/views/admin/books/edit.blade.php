...
@section('title', 'Edit Buku — Admin Librova')
@section('header-title', 'Edit Buku')

@push('styles')
<style>
    /* Styling sama persis seperti create, hanya tambahkan jika perlu */
    .form-card {
    ...
    .current-cover {
        width: 80px; height: 115px; border-radius: 8px; object-fit: cover;
        ...
        <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input class="form-input" type="text" name="title" value="{{ old('title', $book->title) }}" required>
                </div>
                <div class="form-group">
                    <label class="form-label">Penulis</label>
                    <input class="form-input" type="text" name="author" value="{{ old('author', $book->author) }}" required>
                </div>
            </div>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input class="form-input" type="text" name="isbn" value="{{ old('isbn', $book->isbn) }}">
                </div>
                <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea class="form-textarea" name="description" rows="4">{{ old('description', $book->description) }}</textarea>
            </div>

            <div class="form-grid">
                <div class="form-group">
                    <label class="form-label">Bahasa</label>
                    <input class="form-input" type="text" name="language" value="{{ old('language', $book->language) }}" required>
                </div>
                <div class="form-group">
                    <label class="form-label">Tahun Terbit</label>
                    <input class="form-input" type="number" name="published_year" value="{{ old('published_year', $book->published_year) }}">
                </div>
            </div>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input class="form-input" type="number" name="total_pages" value="{{ old('total_pages', $book->total_pages) }}">
                </div>
                <div class="form-group">
                    <label class="form-label">Status</label>
                    <select class="form-select" name="status">
                        <option value="active" {{ old('status', $book->status) == 'active' ? 'selected' : '' }}>Aktif</option>
                        <option value="inactive" {{ old('status', $book->status) == 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-grid">
                <div class="form-group">
                    <label class="form-label">Cover Buku (biarkan kosong jika tidak diganti)</label>
                    @if($book->cover_image)
                        <img src="{{ Storage::url($book->cover_image) }}" class="current-cover" alt="Cover saat ini">
                    @endif
                    <input class="form-input" type="file" name="cover_image" accept="image/*" style="padding:9px;">
                </div>
                <div class="form-group">
                    <label class="form-label">File PDF (biarkan kosong jika tidak diganti)</label>
            ...
            <div style="display:flex;gap:20px;flex-wrap:wrap;margin-bottom:24px;">
                <label class="form-check">
                    <input type="checkbox" name="is_downloadable" value="1" {{ old('is_downloadable', $book->is_downloadable) ? 'checked' : '' }}>
            ...
            <div style="display:flex;gap:12px;justify-content:flex-end;">
                ...
                <button type="submit" class="btn-save"><i class="bi bi-check-lg"></i> Update Buku</button>
            </div>
        </form>
    </div>
</div>
